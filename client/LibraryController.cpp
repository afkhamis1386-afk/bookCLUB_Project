...
void LibraryController::reorderShelfBooks(int shelfId, const QVariantList &bookIds) {
    ...
    if (!networkManager->isConnected()) {
        emit shelfBooksReorderFailed("اتصال به سرور برقرار نیست");
        return;
    }
...
void LibraryController::onResponseReceived(RequestType type, const Response &response) {
    ...
    case RequestType::ReorderShelves:
        if (response.isSuccess())
            emit shelvesReordered(response.getMessage());
        else
            emit shelvesReorderFailed(response.getMessage());
        break;
    case RequestType::ReorderShelfBooks:
        if (response.isSuccess())
            emit shelfBooksReordered(response.getMessage());
        else
            emit shelfBooksReorderFailed(response.getMessage());
        break;
