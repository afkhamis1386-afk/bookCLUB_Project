...
Notification::Notification(int notificationId, int userId, NotificationType type, const QString &title, const QString &message, bool read, const QDateTime &createdAt, int targetId, int senderId)
    : notificationId(notificationId), userId(userId > 0 ? userId : -1), targetId(targetId),
    senderId(senderId), notificationType(type), read(read), createdAt(createdAt) {
    if (!setTitle(title))
        this->title = "اعلان سیستم";
    if (!setMessage(message))
        this->message = ".جزئیاتی ارائه نشده است";
...
QString Notification::getRelativeTime() const {
    ...
    if (seconds < 3600) return QString::number(seconds / 60) + " دقیقه پیش";
    if (seconds < 86400) return QString::number(seconds / 3600) + " ساعت پیش";
    if (seconds < 604800) return QString::number(seconds / 86400) + " روز پیش";
    return createdAt.toString("yyyy/MM/dd");
}
QDataStream &operator<<(QDataStream &out, const Notification &n) {
    out << n.notificationId << n.userId << n.targetId << n.senderId
        << static_cast<int>(n.notificationType) << n.title << n.message
        << n.read << n.createdAt;
    return out;
}
QDataStream &operator>>(QDataStream &in, Notification &n) {
    int typeInt;
    in >> n.notificationId >> n.userId >> n.targetId >> n.senderId
        >> typeInt >> n.title >> n.message >> n.read >> n.createdAt;
